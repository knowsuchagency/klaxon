...
def klaxon(message: str = "", title="Klaxon", subtitle="", sound=""):
    ...
    message, title, subtitle, sound = map(
        shlex.quote, (message, title, subtitle, sound)
    )

    sp.run(
        shlex.split(
            f"""osascript -e 'display notification "{message}" with title "{title}" subtitle "{subtitle}" sound name "{sound}"'"""
        )
    )
